...
HRESULT STDMETHODCALLTYPE SumatraUIAutomationTextRange::MoveEndpointByUnit(TextPatternRangeEndpoint endpoint, TextUnit unit, int count, int *moved)
{
    ...
    public:
        ...
        // do the moving
        int Move(int count, SumatraUIAutomationTextRange* target_, int* target_page_, int* target_glyph_) {
            target = target_;
            target_page = target_page_;
            target_glyph = target_glyph_;
...
HRESULT STDMETHODCALLTYPE SumatraUIAutomationTextRange::GetChildren(SAFEARRAY **children)
{
    ...
    while (it) {
        if (it->GetPageNum() >= startPage && it->GetPageNum() <= endPage) {
            ...
            HRESULT hr = SafeArrayPutElement(psa, &index, static_cast<IRawElementProviderSimple*>(it));
            CrashIf(FAILED(hr));
            it->AddRef();
        }
